{ clobberDestination =
      ./clobberDestination sha256:b6e3d069effc85fbf49b9e81cec8db55c4896b7e9032f920f9b991aae0aa2c66
    ? ./clobberDestination
, ignorePatterns =
      ./ignorePatterns sha256:a088d5df5be35cd12afa08021010992475cb7287035158d6c5e7bbaa7d0defcb
    ? ./ignorePatterns
, keepInputCopy =
      ./keepInputCopy sha256:4b472bee5a7dd511f5167b05dcfc8ac1feee83519f358fb7c23cc734e19fa58a
    ? ./keepInputCopy
, LogLevel =
      ./LogLevel/package.dhall sha256:0a8db2fca7baf776d41672f00c5b8ad8e58b0ab922d2fe5f3b13023d55b8ce34
    ? ./LogLevel/package.dhall
, LogTo =
      ./LogTo/package.dhall sha256:e0b8be744b92ddb5e0370d6d218580218e39c72abe05a1e58a2f9a7f096f6fc1
    ? ./LogTo/package.dhall
, newOutputFormat =
      ./newOutputFormat sha256:9da4e4fd9b80bc4ea341b54ada243e3c15a0beb7745f7fa5437460a852e73cad
    ? ./newOutputFormat
, renaming =
      ./renaming sha256:cce8a41eff204ff7ce0a961a4849815a46af6578c2421e48ee82f96c71708b97
    ? ./renaming
}
