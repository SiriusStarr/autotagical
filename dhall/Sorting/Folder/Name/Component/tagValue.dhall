{-
# Sorting.Folder.Name.Component.tagValue
...
tag is present on the file (whether the folder itself has the predicate
or placing it inside a conditional component).  A warning will be logged and the
file will fail sorting if the tag value of a tag not on a file is requested.
...
in  Autotagical.Sorting.Folder.Name.Component.tagValue
      (Autotagical.Tags.tag "tag1")
```
